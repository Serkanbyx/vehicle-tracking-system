...
test.describe("Simulator E2E — Live markers + alerts", () => {
  test("Dashboard shows moving markers after simulator starts", async ({
    page,
  }) => {
    ...
    const mapCanvas = page.locator("canvas.maplibregl-canvas").or(
      page.locator('[class*="maplibregl-map"]'),
    );
    await expect(mapCanvas).toBeVisible({ timeout: 15_000 });

    /* ── Check for vehicle markers or status indicators ── */
    const markerOrStatus = page.locator('[class*="vehicle-marker"]').or(
      page.locator('[data-testid*="vehicle"]'),
  ...
  test("Speed spike triggers alert visible on alerts page", async ({
    page,
  }) => {
    await login(page, ADMIN);
    ...
    const alertsHeading = page.getByRole("heading", { name: /uyarı|alert/i }).or(
      page.locator("text=Uyarılar"),
    );
    await expect(alertsHeading).toBeVisible({ timeout: 10_000 });

    /* ── Check for speed alerts if any exist ── */
    const speedAlerts = page.locator("text=speed").or(
      page.locator("text=hız"),
    );
